...
|---|---|---|
| `0.01` | the plugin default, and every Tier 1 chapter | almost plain BIC; permissive, tends to dense networks |
...
that must be reported**, in the same way a significance threshold is. On the
300-ASV problem the selected network is reported to flip between a dense
solution, a moderate one and the empty graph over a very narrow window of
$\gamma$ — those figures are pending recompute, but the qualitative point stands:
a network reported without its $\gamma$ is not reproducible. See
[Selecting lambda](../04_highdim_atacama/02_model_selection.md).
